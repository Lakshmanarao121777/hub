    ...
    <table>
      <thead>
        <tr>
          ...
          <th>Subject Name <?php if ($_SESSION['USER_DESIGNATION'] == 'hod' || $_SESSION['USER_DESIGNATION'] == 'admin') {echo '/ Faculty Name';}?></th>
          <th>Unit-Name</th>
        </tr>
      </thead>
      <tbody>
...
while ($row = $q->fetch()): $count++;?>
	        <tr>
	          ...
	          <td style="text-align: center;"><?php echo $row['subName']; ?><?php if ($_SESSION['USER_DESIGNATION'] == 'hod' || $_SESSION['USER_DESIGNATION'] == 'admin') {echo '</br><b> Faculty Assigned : ' . $row['userName'] . '</b>';}?></td>
	          <td style="padding: 0;">
	              <?php
    try
        {
    ...
    while ($rowss = $qss->fetch()): $countss++;
		                        ...
		                        <tr>
		                          ...
		                          <td style="width:25%;text-align:center;border-left:0;border-top:0;">
		                            <span id="<?php echo 'unitnameedit' . $sss; ?>">
		                              <input type="hidden" value="<?php echo $rowss['unitName']; ?>" id="<?php echo 'unitnameeditval' . $sss; ?>"><?php echo $rowss['unitName']; ?></span>
		                            <button class="btn btn-warning btn-sm" style="float:right;margin:1px;" onclick="deleteUnits('<?php echo $sss; ?>')"><span class="fa fa-trash"></span></button>
		                            <span id="<?php echo 'unitnameeditbtn' . $sss; ?>"><button class="btn btn-primary btn-sm" style="float:right;margin:1px;" onclick="editUnits('<?php echo $sss; ?>')"><span class="fa fa-pencil-alt"></span></button></span></td>
        ...
        while ($rowssa = $qssa->fetch()): $countssa++;?>
                        ...
                        ?>
			                                          ...
			                                          <td style="width:55%;border-right: 0;border-top:0;">
			                                            <div class="inp" style="width:32%;font-size:14px;padding:0;">
			                                              <div class="inp_name" style="width:100%;font-size:14px;">Status : </div>
			                                              <div class="inp_value" style="width:100%;font-size:15px;"><select id="<?php echo 'currentstatus' . $modulesno; ?>"><option value="Pending">Pending</option><option value="Completed">Completed</option></select></div>
			                                            </div>
			                                            <div class="inp" style="width:68%;font-size:14px;padding:0;">
			                                              <div class="inp_name" style="width:100%;font-size:14px;padding:0;">Completed On : </div>
			                                              <div class="inp_value" style="width:100%;font-size:14px;padding:0;">
			                                                <input type="hidden" id="<?php echo 'userIDdep' . $modulesno; ?>" value="<?php echo $row['userId']; ?>">
			                                                <input type="date" id="<?php echo 'currentDate' . $modulesno; ?>" style="width:65%">
			                                                <button class="btn btn-sm btn-warning" onclick="DeleteModuless('<?php echo $modulesno; ?>')"  style="float:right;margin:0 1px;"><span class="fa fa-trash"> </span></button>
			                                                <button class="btn btn-sm btn-success" onclick="updateModuless('<?php echo $modulesno; ?>')" style="float:right"><span class="fa fa-check"> </span></button></div>
			                                            </div>
			                                          </td>
			                                        </tr>
			                                    <?php
                    ...
                    ?>
		                                  <tr style="border:0;">
		                                    <td colspan="4" style="border:0;">
		                                      <input type="hidden" id="<?php echo 'userNmaekk_' . $countss . '_' . $count; ?>" value="<?php echo $row['userId']; ?>">
		                                      <input type="text" placeholder="Module Name" id="<?php echo 'moduleNamekk_' . $countss . '_' . $count; ?>">
		                                      <?php $u = $rowss['sno'];
                    $klr = $countss . '_' . $count;?>
		                                      <button class="btn btn-sm btn-success" onclick="addnewModule('<?php echo $u; ?>','<?php echo $klr ?>')"><span class="fa fa-plus"> Module</span></button>
		                                    </td>
		                                  </tr>
		                                  </tbody>
		                                    </table><?php
    ...
    endwhile;
            ?>
	                  <tr>
	                    <td colspan="4" style="border:0;">
	                      <input type="text" placeholder="Unit Name" id="<?php echo 'unitNamekk_' . $count; ?>">
	                      <input type="hidden" id="userIdkk" value="<?php echo $row['userId']; ?>">   <?php $subcode = $row['subCode'];?>
	                      <button class="btn btn-sm btn-success" onclick="addnewUnit('<?php echo $subcode; ?>','<?php echo $count; ?>')"><span class="fa fa-plus"> Unit</span></button>
	                    </td>
	                  </tr>
	                  </tbody>
	                    </table><?php
