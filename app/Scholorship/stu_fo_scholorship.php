...
<body>
    ...
    <script type="text/javascript">
        ...
        function getScr(userId){
            ...
            var isEligble=getColValuess("students_details",userId,"scholoshipEligible");
            var currentYear=getColValuess("students_details",userId,"currentYear");
            if(currentYear=="PUC-1") var years=1;if(currentYear=="PUC-2") var years=2;
            if(currentYear=="ENGG-1") var years=3;if(currentYear=="ENGG-3") var years=5;
            if(currentYear=="ENGG-2") var years=4;if(currentYear=="ENGG-4") var years=6;
            alert(isEligble);
            if(isEligble == "Yes")
            {
                var msg=SendToPhp(values,"../../php/controllers/Student_New.php");
        ...
        function getScrSec(userId){
            ...
            var isEligble=getColValuess("students_details",userId,"scholoshipEligible");
            var currentYear=getColValuess("students_details",userId,"currentYear");
            var fee=[36000,36000,40000,40000,40000,40000];
            if(currentYear=="PUC-1") var years=1;if(currentYear=="PUC-2") var years=2;
            if(currentYear=="ENGG-1") var years=3;if(currentYear=="ENGG-3") var years=5;
            if(currentYear=="ENGG-2") var years=4;if(currentYear=="ENGG-4") var years=6;
            for(var tt=0;tt<years;tt++){ta+=fee[tt];}
            var paid=getrowSum("students_chalan","<?php echo $userId; ?>","amount");
            var tr=0,tp=paid,td=0,ta=0;
            if(isEligble =='Yes')
            {  
              var msg=SendToPhp(values,"../../php/controllers/Student_New.php");
              var obj = JSON.parse(msg);
              ...
              for(var mm=0;mm<years;mm++){
                ...
                oldadate +=   '<td>0</td>';
                oldadate +=   '<td>'+ashu+' Yr</td>';
                oldadate +=   '<td>'+fee[mm]+'</td>';
                var amtReleased =0;
                oldadate +=   '<td>'+amtReleased+' </td>';
                tr +=amtReleased;
                var due =fee[mm]-amtReleased;
                oldadate +=   '<td>'+Math.round(due*100)/100+' </td>';
                if(paid >= due ){
                  oldadate +=   '<td>'+Math.round(due*100)/100+' </td>';
                }
            ...
            td=Math.round((ta-tr)*100)/100;
            oldadate +='<tr><td colspan="5"style="text-align:right;"> Total </td> <td>'+ta+'</td> <td>'+tr+'</td> <td>'+td+'</td> <td>'+tp+'</td> <td>'+Math.round((td-tp)*100)/100+'</td></tr>';
