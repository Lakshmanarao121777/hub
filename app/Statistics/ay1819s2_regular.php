    	...
    	<table class="table table-hover table-bordered">
    		<thead>
    			<tr>
	    			...
	    			<th colspan="3">MM</th>
            <th colspan="3">ME</th>
            <th colspan="3">PUC-1</th>
            <th colspan="3">PUC-2</th>
    			...
    			<?php for ($i = 0; $i < sizeof($years); $i++) {	?>
    					<tr>
    						...
    						<?php $f=0;$m=0;for ($k = 0; $k < sizeof($deptd); $k++) {		?>
									<td><?php $fd = $designation->getCountDistinct("ay1819s2reg", "Department='$deptd[$k]' and gender= 'FEMALE' and currentYear ='$years[$i]' ", "userId");$f +=$fd;		echo $fd;?> </td>
		    					<td><?php $md = $designation->getCountDistinct("ay1819s2reg", "Department='$deptd[$k]' and gender= 'MALE' and currentYear ='$years[$i]' ", "userId");$m += $md;echo $md;?></td>
        ...
        <table class="table table-hover table-bordered" >
          ...
          <tbody>
            <?php for ($y = 0; $y < sizeof($years) ; $y++) { ?>
    	    		<tr>
                <?php $ff = $designation->getDistinct("ay1819s2reg", " currentYear ='$years[$y]' and Department='$deptd[$cyc]' ", "subName");$sff=sizeof($ff['Message']) +1 ; ?>
    	    			<th rowspan="<?php echo$sff;?>" style="vertical-align: middle;"> <?php echo $years[$y]; ?>  </th>
                <?php if(sizeof($ff['Message'])==1){ echo'
                  <tr>
              ...
              <?php 
                $ff = $designation->getDistinct("ay1819s2reg", "currentYear ='$years[$y]' and Department='$deptd[$cyc]' ", "subName");
                for($i=0;$i<sizeof($ff['Message']);$i++){
                  echo"<tr>";
                   ?>
                   <td><?php echo $designation->getColValue("ay1819s2reg","subName = '".$ff['Message'][$i][0]."'", 'subCode'); ?></td>
                  <td> <?php echo $deptd[$cyc]; ?> </td>
                  <?php print_r("<td>". $ff['Message'][$i][0]."</td>") ; ?>
                  <td><?php echo $designation->getColValue("ay1819s2reg","subName = '".$ff['Message'][$i][0]."'", 'subCredit'); ?></td>

                  <td><?php $fd = $designation->getCountDistinct("ay1819s2reg", "Department='$deptd[$cyc]' and gender= 'FEMALE' and currentYear ='$years[$y]' and subName='".$ff['Message'][$i][0]."' ", "userId");    echo $fd;?> </td>
                  <td><?php $md = $designation->getCountDistinct("ay1819s2reg", "Department='$deptd[$cyc]' and gender= 'MALE' and subName='".$ff['Message'][$i][0]."' and currentYear ='$years[$y]' ", "userId");echo $md;?></td>
                  <td><?php echo $md + $fd; ?></td>
