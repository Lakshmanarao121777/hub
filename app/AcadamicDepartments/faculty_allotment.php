...
<body>
  ...
  <?php include_once("../../php/includes/sidebar_AcadamicDepartments.php"); $user_dept=$_SESSION['USER_DEPARTMENT'];?>
  <div class="container bg_white">
    <?php 
    if($_SESSION['USER_DESIGNATION']!="hod"&&$_SESSION['USER_ID']!="admin")
      {
    ...
    <div id="listofsub">
      <?php 
        ...
        try { 
          $pdo = new PDO("mysql:host=$server_servername;dbname=$server_database", $server_username, $server_password);
          $sql = "SELECT * FROM subjects  where subDept='$user_dept' GROUP BY subYear ORDER BY sno ASC ";
          $q = $pdo->query($sql);
          $q->setFetchMode(PDO::FETCH_ASSOC); 
          $count=0;$snos;
          ?>
                  ...
                  <select id="selectSub">
                    <?php
                      try { 
                        ...
                        $sqlj = "SELECT * FROM subjects where subDept='$user_dept' and subYear='$row[subYear]' ORDER BY sno DESC ";
                        $qj = $pdoj->query($sqlj);
                  ...
                  <select id="selectFac">
                    <?php
                      try { 
                        ...
                        $sqlk = "SELECT * FROM employee_details where department='$user_dept' and status='ACTIVE' ORDER BY sno DESC ";
                        $qk = $pdok->query($sqlk);
